...
interface FeatureManager {
    ...
    sealed class InstallState(val featureInfo: Feature.Info) {
        ...
        class RequiresUserConfirmation(val sender: IntentSender?, featureInfo: Feature.Info) :
...
internal class FeatureManagerImpl(
    ...
    private fun <T : Feature<*>> handleDownloadingState(
        ...
        onStateUpdate(FeatureManager.InstallState.Downloading(progress, featureType.info()))
    }
    ...
    private fun <T : Feature<*>> handleInstallingState(
        ...
        val progress = state.progress()
        onStateUpdate(FeatureManager.InstallState.Installing(progress, featureType.info()))
    }
    ...
    private fun <T : Feature<*>> handleInstalled(
        ...
        onStateUpdate(FeatureManager.InstallState.Installed(featureType.info()))
    }
    ...
    override fun <T : Feature<*>> installFeature(
        ...
        val request = SplitInstallRequest.newBuilder()
            .addModule(featureType.info().name)
            .build()
    ...
    override fun <T : Feature<*>> isFeatureInstalled(featureType: KClass<T>): Boolean {
        return splitInstallManager.installedModules.contains(featureType.info().name)
    }
}